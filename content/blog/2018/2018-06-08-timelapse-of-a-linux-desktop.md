...
aliases:
  - /blog/2018/06/timelapse-of-a-linux-desktop/
slug: timelapse-of-a-linux-desktop
...
wanted to capture the size of one of my monitors (`-s 2560x1440`) which is
pretty straight forward. Unusually, my X display is `:1` (visible by printing
the `$DISPLAY` environment variable) instead of the normal `:0`. Finally,
`x11grab` treats multi-monitor displays as one buffer, so to get my right most
monitor I have to add an X offset of the combined widths of my other two
monitors (that's the `5120,0`).
