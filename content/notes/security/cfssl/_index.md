...
CFSSL supports hardware backed private keys (HSMs), including the
[YubiKey][2]. You could also set it up with a Red October server for private
key management, which could in turn be HSM backed.
...
(specifically the elliptic curve variants in Red Hat based distributions) which
...
future certificates. This config includes profiles for both sub-CA signing and
...
[server-cert.json](server-cert.json)
...
to the specific service and not making it generally trusted across the system as
a whole. This is especially true for this program as without an HSM there is no
protection over any of the private keys.

[1]: https://github.com/cloudflare/cfssl
[2]: {{< ref "/notes/security/yubikey.md" >}}
[3]: https://golang.org/
