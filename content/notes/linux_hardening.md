...
Hardening of all my servers is the largest piece IMHO to the [defence in depth](https://en.wikipedia.org/wiki/Defence_in_depth). It brings to them each a strength to stand on their own. I want to be able to plug each and everyone on a raw internet connection and feel safe knowing that they won't be compromised.
...
This hardening guide started out as a summary and a few updates to the [NSA RedHat 5 Operating System Security Guidelings](http://www.nsa.gov/ia/_files/os/redhat/NSA_RHEL_5_GUIDE_v4.2.pdf). Since then it has grown to include a lot of services that were not mentioned anywhere in there, and a few best practices that I've developed myself over the years of managing Linux boxes.

I've started including information from [National Vulnerability Database (NVD) National Checklist Program Repository](http://web.nvd.nist.gov/view/ncp/repository) hosted by NIST. They aggregate some useful information from a few sites including the [Center for Internet Security (CIS) Security Benchmarks](http://benchmarks.cisecurity.org/en-us/). One more site I've got some information from is the [SANS Information Security Reading Room](http://www.sans.org/reading_room/).
...
* Instructions for validating the configuration
...
Hardware and BIOS's change from machine to machine but there are some quick best practices that should be followed. The BIOS should be hardened and physical security should be assessed.
...
IPv4 and IPv6 traffic have separate firewalls. I have examples and a decent base config on the [IPTables](iptables) page.

## Networking

Follow the [Network](network) guide to configure a static address and gateway.
...
You'll want to make sure you configure [RSyslog](rsyslog), [Logrotate](logrotate), and [Logwatch](logwatch) according to their appropriate guides. Logwatch is not installed in the minimum install and should be installed.

### Configure Root's Mail Recipient

Add the line to following line to "/etc/aliases", replacing `administrator@example.tld` with an administrator's email address. This is where logs will be emailed.
...
If it's particularly pesky about not going away you may need to mask it. This will essentially create a symlink of the service to "/dev/null", allowing it to attempt to execute without actually accomplishing anything.
...
* [SSHd](sshd) (If you follow this guide, you won't be able to SSH into the box until users and groups have been configured)
...
[Mysql](mysql) creates it's user with "/bin/bash" as the shell which shouldn't be (this is just an example).
...
* Remove suid privileges on "/bin/mount" and "/bin/umount" using the following command: "chmod a-s /bin/mount /bin/umount"
* Create "/etc/cron.allow" and "/etc/at.allow" putting "root" in both of them. Set the permissions on the files to "600".
* Restrict permissions on all of the cron tasks using the following command: "chmod -R go-rwx /etc/cron.* /etc/crontab"
* Restrict boot configurations to only be readable / writable by root "chmod 600 /etc/grub.conf"
...
* Ensure there are no dangerous directories in root's path (relative or world-writable)
...
* Add Local [Certificate Authority](certificate_authority)'s Public Key to the Trusted Store
