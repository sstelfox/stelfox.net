...
The base configuration file sets up local logging with standard facility-based
filtering and destinations. You can download it here:
...
to syslog) is auditd's log. The following source configuration can be used to
read that log file: [auditd-source.conf](auditd-source.conf).
...
Place the server certificate at `/etc/syslog-ng/server.crt` and its associated
...
with its hash. The following one liner will create the appropriate hash link
...
You'll also want to generate a custom `dhparam` file using the following
command:
...
`tls` in the configuration. The full TLS listener and network destination
config is available here: [tls-server.conf](tls-server.conf).
...
Not all log generators support sending their logs over the network using TLS
...
The log statements between the two can be combined to simplify the reasoning
around what will end up in the final logs. Filtering of specific sources can be
...
a client certificate and key pair, as well as place the server's issuing cert
(or chain of certs) into `/etc/syslog-ng/ca.d` and perform the same hashing
step on each individual certificate as was done in the server portion.
...
The full TLS client sending configuration is available here:
[tls-client.conf](tls-client.conf).
...
If you have [auditd][2] configured to send its messages to syslog via its
...
listener defined in the secure log reception section on this page.
...
# Info filter is required as audispd will include status reports about its
# operations.
filter auditLogs {
...
# section and can be removed if you don't receive insecure logs.
log {
  source(tlsListener);
  destination(networkAuditFile);
...
I may want to modify the group() and user() variable to drop privileges when I
...
[1]: {{< ref "../../security/certificate_authority/" >}}
[2]: {{< ref "../auditd/" >}}
[3]: {{< ref "../rsyslog/" >}}
[5]: {{< ref "../logrotate" >}}
