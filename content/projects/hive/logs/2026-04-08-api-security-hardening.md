...
This security pass wouldn't have been possible without first cleaning up the pedantic lint issues and expanding test coverage. Auth was optional, password hashing was SHA-256, and the GCRA rate limiter had a bug where it never actually rejected requests, this one I stumbled upon by accident why writing end-to-end tests. The nested `Result<Result<_, NegativeOutcome>, InsufficientCapacity>` return was only being checked at the outer level so every request that should have been limited was getting waved through. Not ideal.
