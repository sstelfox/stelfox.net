...
Hardening of all my servers is the largest piece IMHO to the
[https://en.wikipedia.org/wiki Defence_in_depth defence in depth]. It brings to
them each a strength to stand on their own. I want to be able to plug each and
everyone on a raw internet connection and feel safe knowing that they won't be
compromised. In the event that any section of my defence fails, I'll know that
the rest of my systems are strong enough to hopefully be protected from
whatever the weak link was.
...
This hardening guide started out as a summary and a few updates to the
[http://www.nsa.gov/ia/_files/os/redhat/NSA_RHEL_5_GUIDE_v4.2.pdf NSA RedHat 5
Operating System Security Guidelings]. Since then it has grown to include a lot
of services that were not mentioned anywhere in there, and a few best practices
that I've developed myself over the years of managing Linux boxes.

I've started including information from
[http://web.nvd.nist.gov/view/ncp/repository National Vulnerability Database
(NVD) National Checklist Program Repository] hosted by NIST. They aggregate
some useful information from a few sites including the
[http://benchmarks.cisecurity.org/en-us/ Center for Internet Security (CIS)
Security Benchmarks]. One more site I've got some information from is the
[http://www.sans.org/reading_room/ SANS Information Security Reading Room].
...
best practices that should be followed. Follow the [[Security/BIOS]] notes and
ensure the hardware is [[Security/Physical physically secure]].

## Installation Notes

* Follow the [Partitioning](partitioning) guidelines.
...
[IPTables](iptables) and [IP6Tables](ip6tables) can be found on their
respective configuration pages.

## Networking

Follow the [Networking](networking) guide to configure a static address and
gateway.
...
You'll want to make sure you configure [RSyslog](rsyslog),
[Logrotate](logrotate), and [Logwatch](logwatch) according to their appropriate
guides. Logwatch is not installed in the minimum install and should be
installed.

### Configure Root's Mail Recipient

Add the line to following line to /etc/aliases, replacing
administrator@example.com with an administrator's email address. This is where
logs will be emailed.
...
To re-enable it you'll want to execute the following command:
...
"mask" links the service file to /dev/null preventing manual activation as well
as other services attempting to start it as requirements. If you mask a
critical service that the machine need to boot it will fail and you'll need to
fix this by hand in the debug shell.

### All Server Services

On all of my servers I configure:

* [SSHd](sshd) (If you follow this guide, you won't be able to SSH into the box
  until users and groups have been configured)
* [NTPd](ntpd) or [Chronyd](chronyd) (pick your preference)
* [RSyslog](rsyslog)

## Updates

Updates regardless of whether [KSplice](ksplice) is being used should be done
as soon as possible. For the most part there shouldn't be any reason not to
update automatically unless you are using untrusted repositories. There is a
daemon available [yum-updatesd](yum-updatesd) in the Fedora repositories which
...
entry without a super user password. I've done this in /etc/grub.d/10_linux by
adding " --unrestricted" to the CLASS environment variable around line 29 and
...
permissions. This definitely has larger security implications but here is how
to configure that. This grants unlimited sudo privileges to any user in the
sudoers group. They still need to authenticate however.
...
set to /sbin/nologin with the exception of halt, shutdown, sync which should be
/sbin/halt, /sbin/shutdown, and /bin/sync respectively.

[Mysql](mysql) creates it's user with /bin/bash as the shell which shouldn't be
...
## Not Transferred/Integration

```
== System Hardening ==
  * Change the permissions of the rpm binary (///bin/rpm//) to 0700
  * Remove suid privileges on ///bin/mount// and ///bin/umount// using the following command:<code>[root@localhost ~]# chmod a-s /bin/mount /bin/umount</code>
  * Create ///etc/cron.allow// and ///etc/at.allow// putting 'root' in both of them. Set the permissions on the files to 600
  * Restrict permissions on all of the cron tasks using the following command:<code>[root@localhost ~]# chmod -R go-rwx /etc/cron.* /etc/crontab</code>
  * Restrict boot configurations to only be readable/writable by root<code>[root@localhost ~]# chmod 600 /etc/grub.conf</code>
  * Restrict access to the boot scripts<code>[root@localhost ~]# chmod -R go-rwx /etc/rc.d</code>
  * Change the owner and permission of the sudo binary to be owned by root:sudoers and have permissions 750
...
<code>[root@localhost ~]# find / -type f \( -perm -04000 -o -perm -02000\)</code>

The same goes for unowned files as there should never be any of these:

<code>[root@localhost ~]# find / \( -nouser -o -nogroup \) -print</code>
...
===== Checklist =====
  ...
  * Disable USB Support
    ...
    * If USB devices are completely unecessary, disable USB completely with the 'nousb' option in kernel boot options
  * Set kernel.randomize_va_space to 2, 1 if that's not possible
  * Disable Rebooting When 'ctrl-alt-del' is Pressed
    * /etc/init/control-alt-delete.conf: -exec /sbin/shutdown -r now "Control-Alt-Delete pressed"

==== Updating Software ====
  * Repository Configuration
    * yum install yum-plugin-fastestmirror yum-plugin-security yum-presto yum-plugin-changelog yum-plugin-protectbase
  ...
  * Configuring Automatic Updates
    * [[security:fedora_security_check|Fedora Update & Security Check Script]]
...
==== Account Security ====
  ...
  * Set Password Quality Requirements
    * Either [[security:pam_cracklib|pam_cracklib]] or [[security:pam_passwdqc|pam_passwdqc]]
  * Set Lockouts for Failed Password Attempts
    * [[security:pam_tally2|pam_tally2]]
...
==== Misc ====
  * Add Local [[services:ca|Certificate Authority]]'s Public Key to the Trusted Store
  * Harden [[security:sysctl|sysctl.conf]] Values
